package com.detection.back.controller.user;
...
//            user.setId(CommonUtil.createUserCode());
    ...
    @ResponseBody
    public JsonResult updatePwd(Integer id, String pwd, String newPwd, String newPwd2) {
        JsonResult result = new JsonResult();
        if (id == null || pwd == null || newPwd == null || newPwd2 == null) {
        ...
        User admin = userService.selectOne(id);
        if (admin == null) {
            result.addErrorCode(new ErrorCode(100003, "id有误"));
            return result;
        } else {

            try {
                if (Md5SaltTool.validPassword(pwd, admin.getUserPassword())) {
                    ...
                    result.addErrorCode(new ErrorCode(100002, "旧密码错误"));
                }
            } catch (Exception e) {
                e.printStackTrace();
