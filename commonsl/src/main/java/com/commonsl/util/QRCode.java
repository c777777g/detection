...
public final class QRCode {
     ...
     */
    public static  BufferedImage addLogo(BufferedImage qrPic, File logoPic) {
        try {
             ...
             * 设置logo的大小,最多20%0
             */
            int widthLogo = logo.getWidth(null) > qrPic.getWidth() * 2 / 10 ?
                    (qrPic.getWidth() * 2 / 10) : logo.getWidth(null);
            int heightLogo = logo.getHeight(null) > qrPic.getHeight() * 2 / 10 ?
                    (qrPic.getHeight() * 2 / 10) : logo.getHeight(null);

            // 计算图片放置位置，默认在中间
            int x = (qrPic.getWidth() - widthLogo) / 2;
            int y = (qrPic.getHeight() - heightLogo) / 2;
